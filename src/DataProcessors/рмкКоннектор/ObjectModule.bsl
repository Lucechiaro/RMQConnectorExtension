...
Процедура Инициализировать() Экспорт
	...
	Если ПустаяСтрока(ВерсияКомпоненты) Тогда
		ВызватьИсключение "В общих настройках RMQ Connector не указана используемая версия компоненты!";
	КонецЕсли;		
...
Процедура Подключиться(ВиртуальныйХост) Экспорт
	...
	Исключение
		...
		ВызватьИсключение "Не удалось подключиться к серверу. 
							|Проверьте настройки подключения или доступность сервера";
...
Процедура ОтправитьСообщение(Сообщение) Экспорт
	...
	СтрокаАргументов = ПолучитьАргументыСообщенияВJSON(Сообщение);
	
	Попытка
	
		Компонента.BasicPublish(ИмяТочки, "", СодержимоеСообщения, 0, Ложь, СтрокаАргументов);
...
Функция ПолучитьАргументыСообщенияВJSON(Сообщение)
	
	СоставАргументов = Новый Структура;
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	|	рмкИсходящиеСообщенияАргументы.Ключ,
	|	рмкИсходящиеСообщенияАргументы.Значение
	|ИЗ
	|	Справочник.рмкИсходящиеСообщения.Аргументы КАК рмкИсходящиеСообщенияАргументы
	|ГДЕ
	|	рмкИсходящиеСообщенияАргументы.Ссылка = &Сообщение";
	Запрос.УстановитьПараметр("Сообщение", Сообщение);
	Выборка = Запрос.Выполнить().Выбрать();
	
	Пока Выборка.Следующий() Цикл
		СоставАргументов.Вставить(Выборка.Ключ, Выборка.Значение);
	КонецЦикла;
		
	Возврат рмкОбщегоНазначения.СериализоватьОбъект(СоставАргументов);	
