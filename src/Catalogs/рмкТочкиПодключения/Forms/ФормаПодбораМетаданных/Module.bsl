...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТипЗнч(ВыбранныеМетаданные) = Тип("Массив") И ВыбранныеМетаданные.Количество() > 0 Тогда
...
&НаКлиенте
Процедура ЗаполнитьИзПланаОбмена(Команда)
	
	ИменаПлановОбмена = ПолучитьСписокИменПлановОбмена();
	СписокПлановОбмена = Новый СписокЗначений();
	СписокПлановОбмена.ЗагрузитьЗначения(ИменаПлановОбмена);
	
	Если СписокПлановОбмена.Количество() = 0 Тогда
		
		рмкОбщегоНазначенияКлиентПереопределяемый.СообщитьПользователю("В конфигурации отсутствуют планы обмена!");
		Возврат;
		
	КонецЕсли;	
	
	ООСписокПлановОбменаПослеВыбора = Новый ОписаниеОповещения("СписокПлановОбменаПослеВыбора", ЭтотОбъект);
	//ПоказатьВыборИзСписка(ООСписокПлановОбменаПослеВыбора, СписокПлановОбмена, Элементы.ТаблицаМетаданных);
	ПоказатьВыборИзМеню(ООСписокПлановОбменаПослеВыбора, СписокПлановОбмена, Элементы.ЗаполнитьИзПланаОбмена);
...
&НаСервереБезКонтекста
Функция ПолучитьСписокИменПлановОбмена()
	
	СписокИмен = Новый Массив;
	
	Для Каждого ЭлементМетаданных Из Метаданные.ПланыОбмена Цикл
		СписокИмен.Добавить(ЭлементМетаданных.Имя);
	КонецЦикла;	
...
&НаСервереБезКонтекста
Функция ПолучитьИменаОбъектовИзСоставаПланаОбмена(ИмяПланаОбмена)
	
	СписокИмен = Новый Массив;
	
	Для Каждого ЭлементМетаданных Из Метаданные.ПланыОбмена.Найти(ИмяПланаОбмена).Состав Цикл
		
		ОписаниеМетаданных = Новый Структура("ПолноеИмя,ВыгружатьСДвижениями", ЭлементМетаданных.Метаданные.ПолноеИмя(), Ложь);
		СписокИмен.Добавить(ОписаниеМетаданных);
...
&НаКлиенте
Процедура СписокПлановОбменаПослеВыбора(РезультатВыбора, ДополнительныеПараметры) Экспорт
	
	Если РезультатВыбора = Неопределено Тогда
		Возврат;
	КонецЕсли;	
	
	ИмяПланаОбмена = РезультатВыбора.Значение;
	
	Если Не ПустаяСтрока(ИмяПланаОбмена) Тогда
		
		СписокМетаданных = ПолучитьИменаОбъектовИзСоставаПланаОбмена(ИмяПланаОбмена);
		ОтметитьВыбранныеМетаданные(ЭтотОбъект, СписокМетаданных);
