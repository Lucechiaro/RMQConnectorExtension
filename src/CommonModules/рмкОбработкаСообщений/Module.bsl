...
Процедура ПриЗаписиОбъектаРепликации(Источник, Отказ) Экспорт
	...
	Для Каждого ТочкаПодключения Из ТочкиПодключенияДляОбъекта(Источник) Цикл
		СоздатьИсходящиеСообщения(Источник, ТочкаПодключения);
	КонецЦикла;	
...
Процедура рмкПолучитьСообщенияИзОчередей(СлушателиОчередей = Неопределено) Экспорт
	...
	рмкОбщегоНазначенияПереопределяемый.ПроверитьВозможностьРаботыВКопии();
...
Процедура СоздатьИсходящиеСообщения(ОбъектДляСериализации, ТочкаПодключения) Экспорт
	
	Если Не ПравилаРегистрацииПозволяютСоздатьСообщение(ОбъектДляСериализации, ТочкаПодключения) Тогда
		Возврат;
	КонецЕсли;	
	
	МассивИдентификаторовПолучателей = МассивИдентификаторовПолучателей(ОбъектДляСериализации, ТочкаПодключения);
	ОдноСообщениеДляМножестваПолучателей = рмкОбщегоНазначенияПереопределяемый.ЗначениеРеквизитаОбъекта(ТочкаПодключения, "ОдноСообщениеДляМножестваПолучателей");
	
	Если ОдноСообщениеДляМножестваПолучателей Тогда
		
		ДополнительныеПараметрыСообщения = Новый Структура;
		Аргументы = Новый Структура;
		Аргументы.Вставить("МассивПолучателей", МассивИдентификаторовПолучателей);
		ДополнительныеПараметрыСообщения.Вставить("Аргументы", Аргументы);
		СоздатьИсходящееСообщение(ОбъектДляСериализации, ТочкаПодключения, ДополнительныеПараметрыСообщения);
		
	Иначе
		
		Для Каждого КодПолучателя Из МассивИдентификаторовПолучателей Цикл
			
			МассивКодовПолучателей = Новый Массив;
			МассивКодовПолучателей.Добавить(КодПолучателя);
			
			ДополнительныеПараметрыСообщения = Новый Структура;
			Аргументы = Новый Структура;
			Аргументы.Вставить("МассивПолучателей", МассивКодовПолучателей);
			ДополнительныеПараметрыСообщения.Вставить("Аргументы", Аргументы);
			СоздатьИсходящееСообщение(ОбъектДляСериализации, ТочкаПодключения, ДополнительныеПараметрыСообщения);
...
Процедура СоздатьИсходящееСообщение(ОбъектДляСериализации, ТочкаПодключения, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ПоместитьАргументыВИсходящееСообщение(СообщениеОбъект, ДополнительныеПараметры);
...
Функция МассивИдентификаторовПолучателей(ОбъектДляСериализации, ТочкаПодключения)
	
	МассивИдентификаторов = Новый Массив;
	ИменаРеквизитов = "ИспользоватьДляМножестваПолучателей,ИсточникСпискаПолучателей,ИмяПланаОбмена,УИДУзлаОбмена,ОдноСообщениеДляМножестваПолучателей";
	РеквизитыТочки = рмкОбщегоНазначенияПереопределяемый.ЗначенияРеквизитовОбъекта(ТочкаПодключения, ИменаРеквизитов);
	
	Если РеквизитыТочки.ИспользоватьДляМножестваПолучателей Тогда
		
		Если РеквизитыТочки.ИсточникСпискаПолучателей = ПредопределенноеЗначение("Перечисление.рмкИсточникиСписковПолучателей.ПланОбмена") Тогда
			МассивИдентификаторов = МассивКодовУзловПланаОбмена(РеквизитыТочки.ИмяПланаОбмена);
		ИначеЕсли РеквизитыТочки.ИсточникСпискаПолучателей = ПредопределенноеЗначение("Перечисление.рмкИсточникиСписковПолучателей.ПланОбменаСПравиламиРегистрации") Тогда
			МассивИдентификаторов = МассивКодовУзловПланаОбменаПоПравиламРегистрации(ОбъектДляСериализации, ТочкаПодключения); 	
		ИначеЕсли РеквизитыТочки.ИсточникСпискаПолучателей = ПредопределенноеЗначение("Перечисление.рмкИсточникиСписковПолучателей.МенеджерКонвертации") Тогда
			МассивИдентификаторов = МассивКодовПолучателейИзМенеджераМенеджераКонвертации(ОбъектДляСериализации, ТочкаПодключения);
		Иначе
			МассивИдентификаторов = МассивПолучателейПоУмолчанию();		
		КонецЕсли;	
...
Функция МассивПолучателейПоУмолчанию()
	
	МассивПолучателей = Новый Массив;
	МассивПолучателей.Добавить("default");
		
	Возврат МассивПолучателей;	
	
КонецФункции	

Функция МассивКодовУзловПланаОбмена(ИмяПланаОбмена)
	...
	Запрос.Текст = 
	"Выбрать Код 
	|Из ПланОбмена." + ИмяПланаОбмена + "
	|Где Не ЭтотУзел И Не ПометкаУдаления";
	Выборка = Запрос.Выполнить().Выбрать();
...
Функция МассивКодовПолучателейИзМенеджераМенеджераКонвертации(ОбъектДляСериализации, ТочкаПодключения)
	
	МенеджерыКонвертации = СоздатьМенеджерыКонвертации();
	РеквизитыТочки = рмкОбщегоНазначенияПереопределяемый.ЗначенияРеквизитовОбъекта(ТочкаПодключения,
		"МенеджерКонвертации,ИмяМетодаСпискаПолучателей");
	МенеджерКонвертации = ПолучитьМенеджерКонвертации(МенеджерыКонвертации, РеквизитыТочки.МенеджерКонвертации);
	ПараметрыМетода = Новый Структура("Объект,МассивПолучателей", ОбъектДляСериализации, Новый Массив);
	
	Попытка
		МенеджерКонвертации.ВыполнитьМетод(РеквизитыТочки.ИмяМетодаСпискаПолучателей, ПараметрыМетода);
	Исключение

		ОписаниеОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗаписьЖурналаРегистрации(рмкОбщегоНазначения.ПрефиксСобытияЖурналаРегистрации() 
										+ "Ошибка получения списка получателей из менеджера конвертации", 
								УровеньЖурналаРегистрации.Ошибка, , ТочкаПодключения, ОписаниеОшибки);

	КонецПопытки;

	Возврат ПараметрыМетода.МассивПолучателей;		
	
КонецФункции	

Функция МассивКодовУзловПланаОбменаПоПравиламРегистрации(ОбъектДляСериализации, ТочкаПодключения)
	
	МассивКодов = Новый Массив;
	Отказ = Ложь;
	ИмяПланаОбмена = рмкОбщегоНазначенияПереопределяемый.ЗначениеРеквизитаОбъекта(ТочкаПодключения, "ИмяПланаОбмена");

	Если рмкОбщегоНазначенияПовтИсп.ЕстьОбщийМодуль("ОбменДаннымиСобытия") Тогда

		МассивУзловДоВыполненияПравил = Новый Массив;
		ВыгрузитьУзлыРегистрацииОбъектаВМассив(ОбъектДляСериализации, МассивУзловДоВыполненияПравил);
		ОбъектДляСериализации.ОбменДанными.Получатели.Очистить();
		
		ВыполнитьРегистрациюОбъектаНаУзлахОбмена(ИмяПланаОбмена, ОбъектДляСериализации, Отказ);
		
		МассивУзлов = Новый Массив;
		ВыгрузитьУзлыРегистрацииОбъектаВМассив(ОбъектДляСериализации, МассивУзлов);
		ЗагрузитьУзлыРегистрацииОбъектаИзМассива(ОбъектДляСериализации, МассивУзловДоВыполненияПравил);
		
		Если Не Отказ Тогда
			МассивКодов = ПолучитьМассивКодовУзлов(ИмяПланаОбмена, МассивУзлов);
		КонецЕсли;

	Иначе

		ВызватьИсключение "В системе отсутствуют механизмы регистрации объектов в планах обмена! Проверите настройки подсистемы RMQ!";	
...
Функция ПолучитьМассивКодовУзлов(ИмяПланаОбмена, МассивУзлов)
	
	МассивКодов = Новый Массив;
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ТаблицаПланаОбмена.Код КАК Код
	|ИЗ
	|	ПланОбмена." + ИмяПланаОбмена + " КАК ТаблицаПланаОбмена
	|ГДЕ
	|	ТаблицаПланаОбмена.Ссылка В(&МассивУзлов)";
...
Процедура ПоместитьАргументыВИсходящееСообщение(СообщениеОбъект, ДополнительныеПараметры)
	
	Если рмкОбщегоНазначения.ЕстьСвойствоСтруктуры(ДополнительныеПараметры, "Аргументы")
		И рмкОбщегоНазначения.ЕстьСвойствоСтруктуры(ДополнительныеПараметры.Аргументы, "МассивПолучателей") Тогда
			
		Для Каждого КодПолучателя Из ДополнительныеПараметры.Аргументы.МассивПолучателей Цикл
			
			СтрокаАргумента = СообщениеОбъект.Аргументы.Добавить();
			СтрокаАргумента.Ключ = "ReceivingNode_" + СокрЛП(КодПолучателя);
			СтрокаАргумента.Значение = "true"; // указано строкой для того, чтобы сериализатор не преобразовал в "yes"
...
#Область ОбработкаПравилРегистрации

Функция ПравилаРегистрацииПозволяютСоздатьСообщение(ОбъектДляСериализации, ТочкаПодключения)
	
	МожноСоздатьСообщение = Истина;
	Отказ = Ложь;
	
	ИменаРеквизитовТочки = "ПроверятьПоПравиламРегистрации,ИспользоватьДляМножестваПолучателей,ИмяПланаОбмена,ТипИнформации";
	...
	Если РеквизитыТочкиПодключения.ТипИнформации <> ПредопределенноеЗначение("Перечисление.рмкТипыПередаваемойИнформации.Данные") Тогда
		Возврат Истина;
	КонецЕсли;	
	
	Если РеквизитыТочкиПодключения.ПроверятьПоПравиламРегистрации 
			И Не РеквизитыТочкиПодключения.ИспользоватьДляМножестваПолучателей Тогда
	
		Если рмкОбщегоНазначенияПовтИсп.ЕстьОбщийМодуль("ОбменДаннымиСобытия") Тогда
			
			УзелТочкиПодключения = рмкОбщегоНазначенияПовтИсп.УзелТочкиПодключения(ТочкаПодключения);
			
			МассивУзловДоВыполненияПравил = Новый Массив;
			ВыгрузитьУзлыРегистрацииОбъектаВМассив(ОбъектДляСериализации, МассивУзловДоВыполненияПравил);
			ОбъектДляСериализации.ОбменДанными.Получатели.Очистить();
			ВыполнитьРегистрациюОбъектаНаУзлахОбмена(РеквизитыТочкиПодключения.ИмяПланаОбмена, ОбъектДляСериализации, Отказ);
						
			Если Отказ Или Не ОбъектДляСериализации.ОбменДанными.Получатели.Содержит(УзелТочкиПодключения) Тогда
				МожноСоздатьСообщение = Ложь;	
			КонецЕсли;	
	...
	Возврат МожноСоздатьСообщение;
	
КонецФункции	

Процедура ВыгрузитьУзлыРегистрацииОбъектаВМассив(ОбъектДляСериализации, МассивУзлов)

	МассивУзлов.Очистить();

	Для Каждого Узел Из ОбъектДляСериализации.ОбменДанными.Получатели Цикл
		МассивУзлов.Добавить(Узел);
	КонецЦикла;	

КонецПроцедуры

Процедура ЗагрузитьУзлыРегистрацииОбъектаИзМассива(ОбъектДляСериализации, МассивУзлов)

	ОбъектДляСериализации.ОбменДанными.Получатели.Очистить();

	Для Каждого Узел Из МассивУзлов Цикл
		ОбъектДляСериализации.ОбменДанными.Получатели.Добавить(Узел);
	КонецЦикла;

КонецПроцедуры

Процедура ВыполнитьРегистрациюОбъектаНаУзлахОбмена(ИмяПланаОбмена, ОбъектДляСериализации, Отказ)
	
	МодульСобытийОбмена = рмкОбщегоНазначения.МенеджерОбъектаПоИмени("ОбменДаннымиСобытия");
	МодульСобытийОбмена.МеханизмРегистрацииОбъектовПередЗаписью(ИмяПланаОбмена, ОбъектДляСериализации, Отказ);
