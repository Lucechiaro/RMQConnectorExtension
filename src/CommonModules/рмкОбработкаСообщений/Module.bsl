...
Процедура СоздатьИсходящиеСообщения(ОбъектДляСериализации, ТочкаПодключения) Экспорт
	...
	ОдноСообщениеДляМножестваПолучаетелей = рмкОбщегоНазначенияПереопределяемый.ЗначениеРеквизитаОбъекта(ТочкаПодключения, "ОдноСообщениеДляМножестваПолучателей");
	
	Если ОдноСообщениеДляМножестваПолучаетелей Тогда
	...
	Если рмкОбщегоНазначенияПовтИсп.ЕстьОбщийМодуль("ОбменДаннымиСобытия") Тогда
		...
		Если Не Отказ Тогда
			...
			Для Каждого Узел Из МассивУзлов Цикл
				// TODO переписать на отдельную функцию с одним запросом
				МассивУзлов.Добавить(рмкОбщегоНазначенияПереопределяемый.ЗначенияРеквизитовОбъекта(Узел, "Код"));
			КонецЦикла;	
